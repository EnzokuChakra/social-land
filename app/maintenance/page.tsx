...
function MaintenancePageContent() {
  ...
  const { isMaintenance, message, isLoading, error } = useMaintenance();

  // Redirect if maintenance mode is off or user is admin
  useEffect(() => {
    if (!isLoading && (!isMaintenance || session?.user?.role === 'MASTER_ADMIN')) {
      router.push('/dashboard');
    }
  }, [isMaintenance, session, router, isLoading]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white dark:bg-black">
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-sm text-muted-foreground">Checking maintenance status...</p>
        </div>
      </div>
    );
  }
