  ...
  try {
    ...
    const transformedUserStories = userStories.map(story => ({
      ...
      user: {
        ...
        password: story.user?.password || null,
      ...
      likes: story.likes.map(like => ({
        ...
        user: {
          ...
          password: like.user?.password || null,
      ...
      views: story.views.map(view => ({
        ...
        user: {
          ...
          password: view.user?.password || null,
          bio: view.user?.bio || null,
          createdAt: view.user?.createdAt || new Date(),
          updatedAt: view.user?.updatedAt || new Date(),
          isPrivate: view.user?.isPrivate || false,
          verified: view.user?.verified || false
        } as user
      }))
    })) as StoryWithExtras[];
    ...
    const transformedPosts = posts.map(post => ({
      ...
      comments: post.comments.map(comment => ({
        ...
        user: {
          ...
          createdAt: comment.user?.createdAt || new Date(),
          updatedAt: comment.user?.updatedAt || new Date(),
          isPrivate: comment.user?.isPrivate || false,
          verified: comment.user?.verified || false,
          role: (comment.user?.role || 'USER') as UserRole,
          status: (comment.user?.status || 'NORMAL') as UserStatus
        } as user,
        ...
        replies: comment.replies?.map(reply => ({
          ...
          user: {
            ...
            bio: reply.user?.bio || null,
            createdAt: reply.user?.createdAt || new Date(),
            updatedAt: reply.user?.updatedAt || new Date(),
            isPrivate: reply.user?.isPrivate || false,
            verified: reply.user?.verified || false,
            role: (reply.user?.role || 'USER') as UserRole,
            status: (reply.user?.status || 'NORMAL') as UserStatus
          } as user,
      ...
      likes: post.likes.map(like => ({
        ...
        user: {
          ...
          isPrivate: like.user?.isPrivate || false,
          verified: like.user?.verified || false,
          role: (like.user?.role || 'USER') as UserRole,
          status: (like.user?.status || 'NORMAL') as UserStatus
        } as user
      })),
      savedBy: post.savedBy.map(saved => ({
        ...
        user: {
          ...
          bio: saved.user?.bio || null,
          createdAt: saved.user?.createdAt || new Date(),
          updatedAt: saved.user?.updatedAt || new Date(),
          isPrivate: saved.user?.isPrivate || false,
          verified: saved.user?.verified || false,
          role: (saved.user?.role || 'USER') as UserRole,
          status: (saved.user?.status || 'NORMAL') as UserStatus
        } as user
      })),
      tags: post.tags ? post.tags.map(tag => ({
        ...
        user: {
          ...(tag.user || {}),
          name: tag.user?.name || null,
          username: tag.user?.username || "",
          image: tag.user?.image || null,
          verified: tag.user?.verified || false,
