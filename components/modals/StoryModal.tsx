    ...
    try {
      ...
      if (data.success) {
        ...
        if (success && updatedData) {
          // Update the stories in the modal store
          storyModal.setUserStories(prevStories => {
            const updatedStories = [...prevStories];
            const userIndex = updatedStories.findIndex(s => s.userId === currentStory.user.id);
            if (userIndex !== -1) {
          ...
          // If there are no more stories, close the modal and update UI
          if (updatedData.length === 0) {
            // Dispatch event to update story ring
            ...
            if (currentStoryIndex >= updatedData.length) {
              setCurrentStoryIndex(updatedData.length - 1);
            }
          }
        }
        ...
        >
          ...
          {!currentStory ? (
            ...
            <div className="w-full h-full flex flex-col bg-black" style={{ backgroundColor: '#000000' }}>
              ...
              {/* Stories Container */}
              <div className="flex-1 relative story-content-area">
              ...
              {/* Footer */}
              <div className="footer flex-none p-4 bg-black flex items-center justify-between z-10">
                <div>
                  {currentStory?.user?.id === session?.user?.id && (
                    <button
