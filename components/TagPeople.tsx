...
export default function TagPeople({ onTagsChange, maxTags = 10 }: TagPeopleProps) {
  ...
  useEffect(() => {
    ...
    const searchUsers = async () => {
      ...
      setIsSearching(true);
      setError(null);
      console.log("Starting search for:", debouncedSearch);
      
      try {
        const url = `/api/users/search/following?q=${encodeURIComponent(debouncedSearch)}`;
        ...
        const filteredResults = data.users.filter(
          (user: User) => !taggedUsers.some(tagged => tagged.userId === user.id)
        );
  ...
  const handleTagUser = (user: User) => {
    ...
    console.log("Updated tagged users:", newTaggedUsers);
  ...
  const handleRemoveTag = (userId: string) => {
    console.log("Removing tag for user:", userId);
