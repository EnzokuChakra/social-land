...
function PostView({ id, post }: { id: string; post: PostWithExtras }) {
  ...
  const handleImageDoubleClick = async () => {
    ...
    try {
      ...
      setTimeout(() => setShowHeartAnimation(false), 1700);
      ...
      if (!isAlreadyLiked) {
        // Directly call likePost - the back-end and socket events will handle the update
  ...
  useEffect(() => {
    if (!socket) {
      console.log("[PostView] Socket not available for event listening");
      return;
    }
    
    // Handler for post deletion event
    const handlePostDeleted = (data: { postId: string }) => {
      console.log("[PostView] Received postDeleted event:", data);
      
      // If the deleted post is the one we're viewing, redirect to dashboard
      if (data.postId === id) {
        console.log("[PostView] This post was deleted, redirecting to dashboard");
        ...
        // Close the dialog first to prevent any UI glitches
        setIsOpen(false);
        
        // Use a small delay to ensure the toast is visible
        setTimeout(() => {
          // Use replace to ensure we can't navigate back to deleted content
    ...
    // Add event listener
    console.log("[PostView] Adding postDeleted event listener for post:", id);
    socket.on("postDeleted", handlePostDeleted);
    
    // Cleanup function to remove event listener
    return () => {
      console.log("[PostView] Removing postDeleted event listener");
                ...
                <div className="flex items-center gap-3">
                  <div onClick={handleAvatarClick}>
                    <UserAvatar 
                      ...
                      className="w-8 h-8"
                      showStoryRing={storyRingState.shouldShowStoryRing}
                      hasUnviewedStories={storyRingState.hasUnviewedStories}
                    />
                  </div>
